...
## V2.0版本新增源端语言-->目标语言功能
在传参过程中需要传入源端语言和目标语言参数

## 修复翻译后切分table的BUG
ai_translator/book/content.py中修改set_translation的逻辑
...
- [X] 添加选择翻译目标语言。
- [X] 修复表格翻译过程中的偶发错误问题。
- [X] 修改了table的prompt,解决不翻译table的问题。
...
python ai_translator/main.py --target_language chinese --model_type OpenAIModel --openai_api_key $OPENAI_API_KEY --file_format markdown --book tests/test.pdf --openai_model gpt-3.5-turbo
```
