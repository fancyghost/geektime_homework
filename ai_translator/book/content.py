...
class TableContent(Content):
    ...
    def set_translation(self, translation, status):
        try:
            ...
            # fix wrong column number表数据中不能包含"|",为了安全可以在处理前先把数据中的|替换为其他字符
            #table_data = [row.strip().split() for row in translation.strip().split('\n')]
            data = ''.join(translation.strip())
            lines = data.strip('\n').split('\n')
            table_data = []
            for line in lines:
                # 对每一行按'|'分割，并去除每个元素两端的空格
                parts = [x.strip() for x in line.split('|')]
                # 对分割后的列表，去除首尾的空字符串（因为'|'在行首和行尾导致的空字符串）
                cleaned_parts = list(filter(None, parts))
                row = []
